...
window.YAHOO.namespace("rpc");

if (typeof window.YAHOO.rpc.Service == "undefined" || !window.YAHOO.rpc.Service) {
        ...
        */
        _generateService: function(serviceName, method) {
            ...
            var func = function(oParams, opts) {
                ...
                var envelope = window.YAHOO.rpc.Envelope[method.envelope || smd.envelope];
                ...
                var callback = {
                    success: function(o) {
                        ...
                        if (results.result && results.result.status) {
                            window.YAHOO.lacuna.Game.ProcessStatus(results.result.status);
...
//                console.log('Calling ' + method.name + ' with the parameters of ' + Lang.JSON.stringify(params) + '.'); //debug
                ...
                //window.YAHOO.log('Sending ' + r.data + '.', 'debug', 'RPC._generateService');//debug

                var serialized = envelope.serialize(smd, method, params);
                Lang.augmentObject(r, serialized, true);

                return window.YAHOO.rpc.Transport[r.transport].call(self, r);
            };
        ...
        */
        fetch: function(url, callback) {
            if(window.YAHOO.rpc.Service._smdCache[url]) {
                this._smd = window.YAHOO.rpc.Service._smdCache[url];
                this.process(callback);
            }
            else {
                ...
                Util.Connect.asyncRequest('GET', url, {
                    success: function(o) {
                        try {
                            ...
                            window.YAHOO.rpc.Service._smdCache[url] = this._smd;
                            this.process(callback);
                        }
                        catch(ex) {
                            //window.YAHOO.log(ex);
    ...
    /**
     * window.YAHOO.rpc.Transport
     * @class window.YAHOO.rpc.Transport
     * @static
     */
    window.YAHOO.rpc.Transport = {
        /**
        ...
        */
        "JSONP": function(r) {
            ...
            var fctName = encodeURIComponent("window.YAHOO.rpc.Transport.JSONP.jsonpCallback"+window.YAHOO.rpc.Transport.jsonp_id);
            window.YAHOO.rpc.Transport.JSONP["jsonpCallback"+window.YAHOO.rpc.Transport.jsonp_id] = function(results) {
                if(Lang.isObject(r.callback) && Lang.isFunction(r.callback.success)) {
                    r.callback.success.call(r.callback.scope || this, results);
                }
            };
            window.YAHOO.rpc.Transport.jsonp_id+=1;
    ...
    /**
     * window.YAHOO.rpc.Envelope
     * @class window.YAHOO.rpc.Envelope
     * @static
     */
    window.YAHOO.rpc.Envelope = {
        /**
        * JSON-RPC-2.0 envelope
        * @class window.YAHOO.rpc.Envelope.JSON-RPC-2.0
        * @static
        */
        "JSON-RPC-2.0": {
            /**
    ...
    //assign to global
    window.YAHOO.rpc.Service = Service;

})();
window.YAHOO.register("rpc", window.YAHOO.rpc.Service, {version: "1", build: "0"});
