...
//only load this once
if (typeof window.YAHOO.lacuna.TextboxList == "undefined" || !window.YAHOO.lacuna.TextboxList) {
...
(function () {
    ...
    var Util = window.YAHOO.util,
        Dom = Util.Dom,
        Event = Util.Event,
        Lang = window.YAHOO.lang,
        Lib = window.YAHOO.lacuna.Library,
    ...
    var TBL = function(elInput, oDataSource, oConfigs) {
        ...
        if(Dom.inDocument(elInput)) {
            if(window.YAHOO.lang.isString(elInput)) {
                idPrefix = elInput;
    ...
    Lang.extend(TBL, window.YAHOO.widget.AutoComplete, {
        ...
        */
        ...
        handleResponseSingle : function(sQuery, oResponse, oPayload) {
            if((this instanceof window.YAHOO.widget.AutoComplete) && this._sName) {
                //focus textbox
                this._focus();
        ...
        */
        _populateList : function(sQuery, oResponse, oPayload) {
            ...
            if(ok && !oResponse.error) {
                ...
                /*var isOpera = (window.YAHOO.env.ua.opera);
                ...
                if(nItemsToShow > 0) {
                    ...
                    for(var i = nItemsToShow-1; i >= 0; i--) {
                        ...
                        if(this.resultTypeList) {
                            ...
                            var aResult = [];
                            // Match key is first
                            aResult[0] = (window.YAHOO.lang.isString(oResult)) ? oResult : oResult[sMatchKey] || oResult[this.key];
                            // Add additional data to the result array
                            var fields = this.dataSource.responseSchema.fields;
                            if(window.YAHOO.lang.isArray(fields) && (fields.length > 1)) {
                            ...
                            // No specific fields defined, so pass along entire data object
                                ...
                                // Already an array
                                if(window.YAHOO.lang.isArray(oResult)) {
                                    aResult = oResult;
                                }
                                // Simple string
                                else if(window.YAHOO.lang.isString(oResult)) {
                                    aResult = [oResult];
                                }
                                // Object
                                else {
                        ...
                        elListItem._sResultMatch = (window.YAHOO.lang.isString(oResult)) ? oResult : (window.YAHOO.lang.isArray(oResult)) ? oResult[0] : (oResult[sMatchKey] || "");
    ...
    Lang.augmentObject(TBL, window.YAHOO.widget.AutoComplete);
    ...
    //assign to global
    window.YAHOO.lacuna.TextboxList = TBL;

})();


// Important to register it
window.YAHOO.register("textboxList", window.YAHOO.lacuna.TextboxList, {version: "1.0.0", build: "1"});
